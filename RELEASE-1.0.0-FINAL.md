...
- Provisionamento automático de `profiles` + `organizations` + `provider_settings` para usuários novos, com auto-heal idempotente (trigger em `auth.users` + `public.ensure_profile_and_org()`)
- Migration: `supabase/migrations/20260624000015_user_onboarding.sql` aplicada/validada no projeto Supabase `connektpay` (ref `tecnqmtzdefeacbfdyma`)
...
- Teste: `tests/signup-onboarding.spec.ts` (SUCESSO)
- Evidências: `test-results/signup-onboarding-Cadastro-37cc8-ite-abrir-Dashboard-sem-401/`
